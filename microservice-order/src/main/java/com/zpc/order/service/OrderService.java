...
public class OrderService {
     ...
     * @return
     */
    public Order queryOrderByIdx2(String orderId) {
        Order order = ORDER_DATA.get(orderId);
        if (null == order) {
            return null;
        }
        ...
        for (OrderDetail orderDetail : orderDetails) {
            // 通过商品微服务查询商品详细数据
            //Item item = this.itemService.queryItemById4(orderDetail.getItem().getId());
            Item item = this.itemService.queryItemById5(orderDetail.getItem().getId());
            if (null == item) {
                continue;
            }
            orderDetail.setItem(item);
